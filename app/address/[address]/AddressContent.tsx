...
import { faCopy, faFileAlt, faChevronDown, faChevronUp } from '@fortawesome/free-solid-svg-icons'
import Link from 'next/link'
import { minidenticon } from 'minidenticons'
import toast from 'react-hot-toast'
import { Transaction } from '../../types'
import { getNativeBalance, getTransactionsByAddress, getUserTokens, getTokenData, getTokenHolders, getTokenTransactions } from '../../lib/firebase'
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (type === 'wallet') {
        ...
        const sortedTxs = txs.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())
        
        // Calculate total token value
        const totalValue = tokens.reduce((sum, token) => sum + (Number(token.value) || 0), 0)
        ...
        setWalletData({
          ...
          transactions: {
            latest: sortedTxs[0]?.createdAt ? formatTimeAgo(sortedTxs[0].createdAt.getTime() / 1000) : '',
            first: sortedTxs[sortedTxs.length - 1]?.createdAt ? formatTimeAgo(sortedTxs[sortedTxs.length - 1].createdAt.getTime() / 1000) : '',
                ...
                >
                  <div>
                    ...
                    <div className="font-medium">
                      ${walletData.totalTokenValue} ({walletData.tokenHoldings.length} Tokens)
                    </div>
                  </div>
                  <FontAwesomeIcon 
                    ...
                    <div className="max-h-[400px] overflow-y-auto">
                      {filteredTokens.length > 0 ? (
                        filteredTokens.map((token) => (
                          <div key={`${token.address}_${token.symbol}`} className="p-3 hover:bg-gray-50 border-b border-gray-200">
                            <div className="flex justify-between items-center">
                              ...
                              <div className="text-right">
                                <div>${Number(token.value || 0).toFixed(2)}</div>
                                <div className="text-sm text-gray-500">@{Number(token.price || 0).toFixed(4)}</div>
                              </div>
                            </div>
                          </div>
                        ))
          ...
          <div className="bg-white rounded-lg shadow">
            ...
            <div>
              ...
              <div className="p-4 border-b border-gray-200">
                <div className="flex items-center gap-2">
                  <span>Latest {transactions.length > 25 ? '25' : transactions.length} Transactions from a total of </span>
                  <span className="text-blue-500">{walletData.transactions.total.toLocaleString()}</span>
                  ...
                  <tbody>
                    {transactions.map((tx) => (
                      <tr key={tx.transactionHash} className="border-b border-gray-200 hover:bg-gray-50">
                        <td className="p-3">
                          <Link href={`/tx/${tx.transactionHash}`} className="text-blue-500 hover:text-blue-600">
                        ...
                        <td className="p-3">
                          <Link href={`/block/${tx.blockNumber}`} className="text-blue-500 hover:text-blue-600">
                            {tx.blockNumber}
                          </Link>
                        </td>
                        ...
                        <td className="p-3">
                          <div className="flex items-center gap-1">
                            <Link href={`/address/${tx.fromAddress}`} className="text-blue-500 hover:text-blue-600">
                        ...
                        <td className="p-3">
                          <Link href={`/address/${tx.toAddress}`} className="text-blue-500 hover:text-blue-600">
                            {tx.toAddress.slice(0, 8)}...
                          </Link>
                        </td>
                        <td className="p-3 text-right">
                          {tx.amount} {tx.method === 'Token Created' ? 'DFS' : tx.token?.symbol || 'DFS'}
                        </td>
        ...
        <div className="container mx-auto px-4">
          <div className="bg-white rounded-lg shadow">
            <div className="border-b border-gray-200">
              <div className="flex overflow-x-auto">
                <button 
                  className={`px-4 py-2 ${activeTab === 'transactions' ? 'text-blue-500 border-b-2 border-blue-500' : 'text-gray-600'}`}
                  onClick={() => setActiveTab('transactions')}
                >
                  Transactions
                </button>
                <button 
                  className={`px-4 py-2 ${activeTab === 'holders' ? 'text-blue-500 border-b-2 border-blue-500' : 'text-gray-600'}`}
